...
1. For each function, loops through the **last 13 months** to fetch data for each period.
1. Executes the function dynamically and retrieves **organization unit (OU) data**.
1. Stores or updates the calculated values in the `datavalue` table.
1. Tracks execution time and logs status updates in the `jobconfiguration` table.
1. Handles errors gracefully, ensuring that failures are logged properly.
...
> **Install and Run the SQL File [sql_agg_calculate_and_store-functions_and_procedure.sql](../scripts/sql_agg_calculate_and_store-functions_and_procedure.sql) in your PostgreSQL (PSQL) Database**
...
**Run the SQL file** using the `\i` command from inside `psql`:

CREATE OR REPLACE FUNCTION update_htn_visits_and_ncd_patient_status_with_call_data(new_program_stage_id BIGINT,
                                                             new_program_stage_instance_id BIGINT,
                                                             new_program_instance_id BIGINT, new_eventdatavalues JSONB,
                                                             new_status TEXT,
                                                             new_executiondate TIMESTAMP
) RETURNS VOID AS
$$
DECLARE
...
END IF;
    ...
    ELSEIF (new_program_stage_id =
            ...
            new_status IN ('ACTIVE', 'COMPLETED')) THEN
...
WHERE psi.programinstanceid = new_program_instance_id
...
first_calling_report_data = COALESCE(new_eventdatavalues, '{}'::jsonb) ||
...
WHERE programstageinstanceid = new_program_stage_instance_id;
...
------------------------------------------------------------------------------------------------------------------------
SELECT update_htn_visits_and_ncd_patient_status_with_call_data(programstageid, programstageinstanceid,
